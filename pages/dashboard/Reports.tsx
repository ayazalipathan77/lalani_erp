  ...
  const generateReport = () => {
    ...
    switch (reportType) {
      case 'SALES_SUMMARY':
        ...
        const totalSales = filteredInvoices.reduce((sum, i) => sum + i.total_amount, 0);
        const paidSales = filteredInvoices.filter(i => i.status === 'PAID').reduce((sum, i) => sum + i.total_amount, 0);
        
        metrics = [
          { label: 'Total Revenue', value: `PKR ${totalSales.toLocaleString()}` },
          { label: 'Cash Collected', value: `PKR ${paidSales.toLocaleString()}` },
      ...
      case 'INVENTORY_VALUATION':
        ...
        const totalStockValue = data.reduce((sum, p) => sum + p.total_value, 0);
        const totalItems = data.reduce((sum, p) => sum + p.current_stock, 0);

        metrics = [
          { label: 'Total Inventory Value', value: `PKR ${totalStockValue.toLocaleString()}` },
          { label: 'Total Items in Stock', value: totalItems.toLocaleString() }
        ];
        break;
      ...
      case 'CUSTOMER_BALANCES':
        ...
        data = rawCustomers.sort((a,b) => b.outstanding_balance - a.outstanding_balance);
        
        const totalReceivables = data.reduce((sum, c) => sum + c.outstanding_balance, 0);
        metrics = [
           { label: 'Total Receivables', value: `PKR ${totalReceivables.toLocaleString()}` },
           { label: 'Customers with Balance', value: data.filter(c => c.outstanding_balance > 0).length.toString() }
        ];
        break;
      ...
      case 'VENDOR_BALANCES':
        ...
        data = rawSuppliers.sort((a,b) => b.outstanding_balance - a.outstanding_balance);

        const totalPayables = data.reduce((sum, s) => sum + s.outstanding_balance, 0);
        metrics = [
            { label: 'Total Payables', value: `PKR ${totalPayables.toLocaleString()}` },
            { label: 'Vendors to Pay', value: data.filter(s => s.outstanding_balance > 0).length.toString() }
        ];
        break;
      ...
      case 'EXPENSE_BREAKDOWN':
         ...
         metrics = [
            { label: 'Total Expenses', value: `PKR ${totalExpenses.toLocaleString()}` },
      ...
      case 'TRANSACTION_HISTORY':
         ...
         const totDeb = filteredTrans.reduce((sum, t) => sum + t.debit_amount, 0);
         const totCred = filteredTrans.reduce((sum, t) => sum + t.credit_amount, 0);
         metrics = [
             { label: 'Total Inflow', value: `PKR ${totDeb.toLocaleString()}` },
             { label: 'Total Outflow', value: `PKR ${totCred.toLocaleString()}` },
             { label: 'Net Change', value: `PKR ${(totDeb - totCred).toLocaleString()}` }
         ];
         break;
    }
