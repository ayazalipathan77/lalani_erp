export default (app, pool, logger) => {
    ...
    app.get('/api/discount-vouchers', async (req, res) => {
        try {
            ...
            const countResult = await pool.query('SELECT COUNT(*) as total FROM discount_vouchers WHERE comp_code = $1', [companyCode]);
            const total = parseInt(countResult.rows[0].total);

            const result = await pool.query(
                'SELECT * FROM discount_vouchers WHERE comp_code = $1 ORDER BY voucher_date DESC LIMIT $2 OFFSET $3',
